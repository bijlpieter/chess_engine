...
void evaluation_info(std::string FEN){
    ...
    std::cout << p << std::endl;
    std::vector<PawnInfo> pawn_hash_table(131072);
    std::vector<PawnInfo>* hash = &pawn_hash_table;
    Score white_threats = p.calculate_threats(WHITE);
    Score black_threats = p.calculate_threats(BLACK);
    Score total = p.calculate_score(hash);
    Score material = p.calculate_material(hash);
    ...
    PawnInfo* p_info = p.get_pawn_info(p.state->pawn_key, hash);
    ...
    std::cout << "WHITE: " << p.pawn_score(WHITE, hash);
    std::cout << " BLACK: " << p.pawn_score(BLACK, hash) << std::endl << std::endl;;
